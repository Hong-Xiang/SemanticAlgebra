...
{
    public static ISemantic1<ComVA, TS, TR> Compose<TS, TI, TR>(ISemantic1<ComVA, TS, TI> s, Func<TI, TR> f)
    {
        throw new NotImplementedException();
    }

    public static ISemantic1<ComVA, T, IS<ComVA, T>> Id<T>()
    {
        throw new NotImplementedException();
    }

    public static ISemantic1<ComVA, TS, IS<ComVA, TR>> MapS<TS, TR>(Func<TS, TR> f)
    {
        throw new NotImplementedException();
    }
}

sealed class ComposedSemantic
{
}

interface IComVASemantic<TI, TO>
    : ISemantic1<ComVA, TI, TO>
    , IValSemantic<TI, TO>
    , IAddCSemantic<TI, TO>
{
}
...
static class Test
{
    static void Bar<T>(T val)
    {
        Foo(new Val<T>(val));
    }
    static void Foo<T>(IS<ComVA, T> e)
    {
    }
}
